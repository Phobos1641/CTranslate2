...
if [ "$CIBW_ARCHS" == "aarch64" ]; then
    ...
    (
      ...
      OPENMPI_OWNER="Phobos1641"
      OPENMPI_REPO="oneDNN"
      OPENMPI_TAG="v${OPENMPI_VERSION}-phobos"
      OPENMPI_PREFIX="/usr/local"

      OPENMPI_ASSET="openmpi-${OPENMPI_TAG}-linux-x86_64-glibc2.34.tar.gz"
      OPENMPI_URL="https://github.com/${OPENMPI_OWNER}/${OPENMPI_REPO}/releases/download/${OPENMPI_TAG}"

      OPENMPI_TMP="$(mktemp -d)"
      curl -fsSL -o "${OPENMPI_TMP}/${OPENMPI_ASSET}"        "${OPENMPI_URL}/${OPENMPI_ASSET}"
      curl -fsSL -o "${OPENMPI_TMP}/${OPENMPI_ASSET}.sha256" "${OPENMPI_URL}/${OPENMPI_ASSET}.sha256"
      ( cd "$OPENMPI_TMP" && sha256sum -c "${OPENMPI_ASSET}.sha256" )

      mkdir -p "$OPENMPI_PREFIX"
      tar -xzf "${OPENMPI_TMP}/${OPENMPI_ASSET}" -C "$OPENMPI_PREFIX"
      rm -rf "$OPENMPI_TMP"
    ) || (
      OPENMPI_VERSION=4.1.6
